    ...
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
                ...
                // ];
                // var ignoreForPaletteDefinition = ['StartTimerEvent', 'StartSignalEvent', 'StartMessageEvent', 'StartErrorEvent',
                //     'ServiceTask', 'ScriptTask', 'BusinessRule', 'ReceiveTask', 'ManualTask', 'CamelTask',
                //     'MuleTask', 'SendTask', 'SubProcess', 'EventSubProcess', 'CallActivity', 'ParallelGateway', 'InclusiveGateway',
                //     'EventGateway', 'BoundaryErrorEvent', 'BoundaryTimerEvent', 'BoundarySignalEvent', 'BoundaryMessageEvent',
                //     'BoundaryCancelEvent', 'BoundaryCompensationEvent', 'CatchTimerEvent', 'CatchSignalEvent', 'CatchMessageEvent',
                //     'ThrowNoneEvent', 'ThrowSignalEvent', 'EndErrorEvent', 'EndCancelEvent', 'EndTerminateEvent', 'Pool',
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            // add by zhangyu begin
                            // add properties visible by config file
                            var idWithoutNs = selectedShape.getStencil().idWithoutNs();
                            if (KISBPM.MISROBOT_SHAPE_PROPERTIES_CONFIG[idWithoutNs] != undefined
                                && KISBPM.MISROBOT_SHAPE_PROPERTIES_CONFIG[idWithoutNs][key] != undefined
                                && (KISBPM.MISROBOT_SHAPE_PROPERTIES_CONFIG[idWithoutNs][key] == false)) {
                                continue;
                            }
                            // add by zhangyu end 
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length == 1) {
                    ...
                    ////edit by zhangyu begin///////
                    // comment morph-button 
                    ...
                    // 只有条件分支连线和morph-button对象支持编辑功能
           //          var needShowEditButton = false;
        			// var left = 24;
        			// if (document.getElementById('morph-button').style.display == 'block') {
        			// 	left += 24;
           //              needShowEditButton = true;
        			// } else {
           //              var incommingShapes = selectedShape.incoming;
           //              if (incommingShapes && incommingShapes.length > 0) {
           //                  var incommingStencilItem = $scope.getStencilItemById(incommingShapes[0].getStencil().idWithoutNs());
           //                  if (incommingStencilItem.morphRole == 'GatewaysMorph') {
           //                      needShowEditButton = true;
           //                  }
           //              }
           //          }
           ...
           //  			editButton.style.left = x + left + 'px';
           ...
           //          }
                    ...
                    //     left = 24;
                    // }
                    // deleteButton.style.left = x + left + 'px';
            ...
            // add by zhangyu begin
            $scope.getItemByShape = function(shape) {
                var selectedItem = {'title': '', 'properties': []};
                ...
                for (var i = 0; i < properties.length; i++) {
                    ...
                    if (key === 'oryx-name') {
                        selectedItem.title = shape.properties[key];
                    }
                    ...
                    if (propertyConfig === undefined || propertyConfig === null) {
                        ...
                        if (shape.properties[key] === 'true') {
                            shape.properties[key] = true;
                        }
                        ...
                        // add by zhangyu begin
                        // add properties visible by config file
                        var idWithoutNs = shape.getStencil().idWithoutNs();
                        if (KISBPM.MISROBOT_SHAPE_PROPERTIES_CONFIG[idWithoutNs] != undefined
                            && KISBPM.MISROBOT_SHAPE_PROPERTIES_CONFIG[idWithoutNs][key] != undefined
                            && (KISBPM.MISROBOT_SHAPE_PROPERTIES_CONFIG[idWithoutNs][key] == false)) {
                            continue;
            ...
            $scope.editProperties = function() {
                $scope.safeApply(function () {
                    ...
                    if (shapes && shapes.length == 1)
                    {
                        ...
                        if (stencilItem.type == 'edge') {  // 所选择的是边
                            // if (!$scope.selectedItem.properties[3].hidden) {
                            //     $scope.selectedItem.properties[3].mode = "write";
                            // }
                        } else if (stencilItem.type == 'node') { //所选择的是节点
                        }

                        // Method to open properties editor dialog (used later on)
                        var showPropertiesDialog = function()
                        {
                            $rootScope.currentStencilItem = stencilItem
                            $modal({
                                backdrop: false,
                                keyboard: true,
                                scope: $scope,
                                template: 'editor-app/popups/show-properties.html?version=' + Date.now()
                            });
                        };
        ...
        $scope.propertyClicked = function (index) {
            if (!$scope.selectedItem.properties[index].hidden) {
                ...
                // add by zhangyu begin
                // does not support modify bpmndiagram's name
                if ($scope.selectedShape.getStencil().idWithoutNs() == 'BPMNDiagram'
                    && $scope.selectedItem.properties[index].key == 'oryx-name') {
                    return;
                }
                // add by zhangyu end
